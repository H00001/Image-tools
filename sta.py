...
def get_max_metrics(file_path):
    max_acc, max_nmi, max_ari, max_f1 = -1, -1, -1, -1

    # Read the file and extract metrics from each line
    with open(file_path, 'r') as file:
        ...
        for line in lines:
            ...
            nmi = float(values[1].split()[1])
            ari = float(values[2].split()[1])
            f1 = float(values[3].split()[1])
    ...
    return max_acc, max_nmi, max_ari, max_f1
...
#             print(f"{filename}\t{max_acc}\t{max_ari}\t{max_nmi}")
#             accs.append(max_acc)
#             aris.append(max_ari)
#             nmis.append(max_nmi)
#             f1s.append(max_f1)
